      ...
      <div className="w-[100vh] h-[100vw] -rotate-90 origin-center bg-gray-50 flex flex-col shadow-2xl">
        ...
        <div className="bg-white pb-2 shadow-sm z-10 shrink-0">
          <div className="flex gap-3 px-4 py-3">
            <button onClick={() => navigate("/voice")} className="flex-1 bg-pink-50 p-3 rounded-xl border border-pink-100 flex items-center gap-2 justify-center">
              <Volume2 className="text-pink-500 w-6 h-6"/> <span className="font-bold text-pink-600 text-lg">음성주문</span>
            </button>
            <button onClick={() => navigate("/easy")} className="flex-1 bg-orange-50 p-3 rounded-xl border border-orange-100 flex items-center gap-2 justify-center">
              <Sparkles className="text-orange-500 w-6 h-6"/> <span className="font-bold text-orange-600 text-lg">쉬운주문</span>
            </button>
          </div>
          ...
          <div className="flex gap-2 overflow-x-auto px-4 py-2 no-scrollbar">
            {categories.map(cat => (
              <button key={cat} onClick={() => setActiveCategory(cat)}
                className={`whitespace-nowrap px-6 py-3 rounded-full font-bold text-lg transition-colors ${activeCategory === cat ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-500'}`}>
                {cat}
              </button>
            ))}
          </div>
        ...
        <main className="flex-1 overflow-y-auto p-4 bg-gray-50 no-scrollbar">
          {isLoading ? (
            <div className="h-full flex justify-center items-center text-xl">메뉴를 불러오는 중...</div>
          ) : (
            <MenuGrid items={filteredItems} onItemClick={(item) => {
